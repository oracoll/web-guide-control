...
float positionAccumulator = 0.0; // Accumulator for fractional steps from velocity integration
  ...
  switch(homingState) {
    ...
    case HOMING_CENTER:
      ...
      if (stepper.distanceToGo() == 0) {
        ...
        // LEDs are updated in main loop now
      }
      break;
  ...
  if (menuState != NORMAL && menuState != MENU_EXIT_CONFIRM) {
    ...
    if (encoderDiff != 0) {
      ...
      if (menuState == MENU_BROWSE) {
        ...
        } else if (currentMenuItem > menuTopItem + 1) { // Adjusted for 2 lines
          menuTopItem = currentMenuItem - 1; // Adjusted for 2 lines
        }
        lastMenuTopItem = -1;
      ...
      if (encBtnState == LOW && encoderButtonPrevState == HIGH) {
        ...
        if (held >= SHORT_PRESS_MS && held < LONG_PRESS_MS) {
          if (currentCalibMenuItem == 0) {
            ...
            delay(500);
            startCentering(); // This will display "Centering..."
            wdt_enable(WDTO_4S);
            displayLockoutUntil = currentMillis + DISPLAY_LOCKOUT_MS;
          } else {
            calibState = CALIB_IDLE;
...
void handleToggleButton() {
  ...
  if (toggleBtnState == HIGH && toggleButtonPrevState == LOW) {
    ...
    if (now - toggleButtonDownTime >= DEBOUNCE_MS) {
      ...
      saveSettings();
      // The main loop will handle the display update.
    }
  }
